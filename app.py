...
if st.session_state.first is None:
    ...
    submit = st.button("결정", key="first_choice_button")
...
# 게임 진행
# -----------------------
if st.session_state.first is not None and not st.session_state.game_over:
    ...
    if st.session_state.first == 1:
        my_num = st.selectbox(
            ...
            key=f"my_num_{st.session_state.round}"
        )
        submit = st.button("제출", key=f"submit_{st.session_state.round}")
